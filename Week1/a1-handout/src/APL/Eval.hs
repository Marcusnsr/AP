...
-- Task 2: Implement the Lambda and Apply cases of eval
eval env (Lambda var e1) =
  Right $ ValFun env var e1
eval env (Apply e1 e2) =
  case eval env e1 of
    Left err -> Left err
    Right (ValFun env' var body) ->
      case eval env e2 of
        Left err -> Left err
        Right v -> eval (envExtend var v env') body
    Right _ -> Left "Non-function applied"
-- Task 2 ends here
-- Task 3: Implement the TryCatch case of eval
eval env (TryCatch e1 e2) =
  case eval env e1 of
    Left _ -> 
      case eval env e2 of
        Left _ -> Left "Both expressions failed"
        Right v -> Right v
    Right v -> Right v
-- Task 3 ends here